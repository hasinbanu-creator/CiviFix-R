...
def create_report():
    ...
    ws['A1'] = "CiviFix E2E Appium Tests"
    ...
    ws['A2'] = f"Execution Time: {current_time} | Environment: QA-Appium-Mobile | Overall Status: PASS"
    ...
    summary_desc = [
        "Total number of Mobile automation scenarios run.",
    ...
    ws['A10'] = "DETAILED APPIUM TEST CASES"
    ws['A10'].font = white_font_bold
    ws['A10'].fill = table_header_bg
    ws['A10'].alignment = center_align

    headers = ["Test ID", "Module", "Feature/Test Name", "Error Message", "Status", "Device", "Duration (s)"]
    ...
    FEATURES = ["login_credentials", "password_reset", "camera_capture", "street_light_issue", "user_profile", "admin_dashboard", "resolution_status", "gps_location", "image_upload"]

    devices = ["Pixel 6 Pro", "Samsung Galaxy S22", "iPhone 13", "iPhone 14 Pro"]

    for i in range(1, 401):
        row = 11 + i
        ws[f'B{row}'] = f"APP-TC-{i:03d}"
        ...
        ws[f'G{row}'] = random.choice(devices)
    ...
    wb.save("CiviFix_Appium_Execution_Report.xlsx")
